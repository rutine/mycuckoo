...
if [ -e $CONF_PATH ] ; then
  ...
  for cfg in $CONF_PATH/*.xml
  do  CLASSPATH="$CLASSPATH:$cfg"
  done
  for cfg in $CONF_PATH/*.properties
  do  CLASSPATH="$CLASSPATH:$cfg"
  done
  for cfg in $CONF_PATH/*.yml
  do  CLASSPATH="$CLASSPATH:$cfg"
  done
  ...
  $JAVA $JAVA_OPTS $JAVA_DEBUG_OPT $MYCUCKOO_OPTS -classpath .:$CLASSPATH -jar mycuckoo-gateway.jar 1>>$BASE_PATH/logs/std.out 2>&1 &
