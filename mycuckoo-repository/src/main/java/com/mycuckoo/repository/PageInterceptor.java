	...
	protected Object extractRealParameterObject(Object parameterObj) {
		if (parameterObj instanceof Map<?, ?>) {
			...
			while (it.hasNext()) {
				...
				if ("0".equals(keyStr) || "1".equals(keyStr) || (entry.getValue() instanceof Pageable)) {
					it.remove();
				}
			}
			
			if(parameterMap.size() == 1 && parameterMap.containsKey("param1")) {
				return parameterMap.get("param1");
			}
		}
