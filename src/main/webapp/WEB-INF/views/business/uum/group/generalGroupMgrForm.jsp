...
<body>
			...
			<div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main mycuckoo-main">
				<div class="row">
					<div class="col-sm-12 col-md-12">
						...
						<h3 class="page-header">普通组编辑/查看</h3>
						<!-- 表单操作按钮 -->
						...
						<form name="editForm" action="${action}Form" method="post">
							...
							<table class="table">
								...
								<tr>
									<td><label>成员类型</label></td>
									<td colspan="3">
										<label class="radio-inline"><input type="radio" name="groupMemberType" value="user" 
												<c:if test="${'user' eq groupMemberType  or empty groupMemberType }">checked</c:if>> 用户</label>
										<label class="radio-inline"><input type="radio" name="groupMemberType" value="role" 
												<c:if test="${'role' eq groupMemberType}">checked</c:if>> 角色</label>
									</td>
								</tr>
								<tr>
									<td><label>机构</label></td>
									<td>
										<input type=hidden  name="generalGroupOrgId"  />
										<input type=text  name="generalGroupOrgName" />
									</td>
									<td><label>角色</label></td>
									<td><select name="generalGroupRoleId" <c:if test="${'role' eq groupMemberType}">class="hide"</c:if>></select></td>
								<tr>
									<td></td>
									<td colspan="3">
										<table>
													...
													<div class="select-side">
														...
														<select id="right_multi_select" multiple="multiple" style="width: 200px; height:200px; border: 1px solid #0088CC;">
															<c:forEach var="groupMember" items="${group.uumGroupMembers}">
																<option value="${groupMember.groupMemberType}_${groupMember.memberResourceId}" selected>${groupMember.memberResourceName}</option>
															</c:forEach>
														</select>
													</div>
												</td>
							...
							<c:forEach var="groupMember" items="${group.uumGroupMembers}">
								<input type="hidden" name="memberIdList" value="${groupMember.memberResourceId}">
							</c:forEach>
						</form>
					</div>
				</div>
	...
	<script type="text/javascript">
		jQuery(function($) {
			...
			// 选择机构
			var $memberType = $form.find('input[name=groupMemberType]');
			var $orgId = $form.find('input[name=generalGroupOrgId]');
			var $orgName = $form.find('input[name=generalGroupOrgName]');
			var $roleId = $form.find('select[name=generalGroupRoleId]');
			var _setting = {
					...
					callback : {
						onClick : function(evane, treeId, treeNode) {
							...
							// 加载角色
							if($form.find('input[name=groupMemberType]:checked').val() == 'role') {
								userFind();
							} else {
			...
			function onBodyDown(event) {
				if (!(event.target.name == 'generalGroupOrgName' 
			...
			function userFind() {
				...
				params.memberType = $form.find('input[name=groupMemberType]:checked').val();
				$.getJSON('queryUserList', params, function(json) {
					var _html = '';
					$.each(json, function() {
						_html += '<option value="' + (params.memberType + '_' + this.userId) + '">' + this.userName + '</option>';
					});
			...
			$main.on('click', '.to-right', function() {
				$leftSel.find('option:selected').each(function() {
					$form.append($('<input type="hidden" name="memberIdList"/>').val(this.value));
					$(this).remove().appendTo($rightSel);
				});
			});
			$main.on('click', '.to-left', function() {
				$rightSel.find('option:selected').each(function() {
					$form.find('input[name=memberIdList][value=' + this.value + ']').remove();
